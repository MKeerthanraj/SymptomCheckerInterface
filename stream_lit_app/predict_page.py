...
def show_predict_page():
    st.title("Symptom Checker Interface")
    st.write("### Tell us your symptoms so that we can identify the cause.")
    symptoms_input=list(st.multiselect("Select Symptoms",symptoms_list))
    predict_bool = st.button("Diagnose")
    if predict_bool:
        input=','.join(symptoms_input)
        input=input.replace('_',' ').title()
        diagnosis = predictDisease(input)
        diag_dict = dict(diagnosis)
        st.write("# Here's your diagnosis!!")
        st.write("## You might be suffering from:")
        st.subheader(diag_dict['final_prediction'])